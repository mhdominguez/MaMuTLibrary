...
# usage: perl MaMuT_dataset_split_random_subgroups.pl dataset_mamut.xml X
#	where X is the desired number of bins
...
sub main {
	...
	if ( open(FILE, "<$path/$dataset_file" ) ) {
		print "Splitting dataset $dataset_file into $num_bins bins, randomly subdivided.  Can change number of bins from command line, in parameter that follows dataset filename.\n";
		flock(FILE, LOCK_EX);
		while( <FILE> ) {
			chomp;
