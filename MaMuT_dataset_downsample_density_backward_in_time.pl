...
sub main {
	...
	for ( my $k=$index; $k>=0; $k-- ) {
		if ( $lines_spots[$index][$k] =~ /<Spot/ && $lines_spots[$index][$k] =~ /ID=\"(\d+)\"/ ) {
			...
			if ($cell_densities[$index][$k] =~ /\d/) { #got a valid cell
				$this_bin = 1; #throw away by default
				$this_bin = 0 if ( $cell_densities[$index][$k] =~ /\d/ &&  $cell_densities[$index][$k] > $timepoint_density_cutoffs[$index] * rand() * $split_fold_inverse );
...
#	}
	...
	for (my $l=0; $l<$num_bins; $l++ ) {
		...
		if ( open(FILE, ">$path/$dataset_file\.dedensified$split_fold\.rev\.xml" ) ) {
			flock(FILE, LOCK_EX);
			...
			print "Error writing to $path/$dataset_file\.dedensified$split_fold\.rev\.xml!\n";
			return;
		}		
	}
}
