...
# usage: perl MaMuT_dataset_split_XYZ_coordinate.pl mamut_dataset.xml bin=2 med_z #med_z can be replaced with mean_x, mean_y, mean_z, med_x, or med_y
...
sub main {
	...
	for ( my $k=scalar(@{$lines_spots[0]})-1; $k>=0; $k-- ) {
		if ( $lines_spots[0][$k] =~ /<Spot/ && $lines_spots[0][$k] =~ /ID=\"(\d+)\"/ ) {
			#capture ID
			$this_spot_id = $1;
			if ($lines_spots[0][$k] =~ /POSITION_${this_axis}=\"([+-]?\d+(\.?\d*))\"/ ) {
				$captured = $1;
				$this_bin - 1;
