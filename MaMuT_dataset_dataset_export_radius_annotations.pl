...
sub main {
	...
	for (my $t=0; $t<scalar(@frames_spots_ann); $t++ ) {
		...
		#print "A: \$t = $t, \$u = $pre_frame_pointer " . scalar(@{$lines_spots_pre_names[$pre_frame_pointer]}) . " " . scalar(@{$lines_spots_ann_ids[$pre_frame_pointer]}) . " \n";
...
#		#now, match all spots
		for ( my $k=0; $k<scalar(@{$lines_spots_ann_ids[$t]}); $k++ ) {
			...
			for ( my $j=0; $j<scalar(@{$lines_spots_pre_ids[$pre_frame_pointer]}); $j++ ) {
				if ( $lines_spots_pre_names[$pre_frame_pointer][$j] eq $lines_spots_ann_names[$t][$k] && $lines_spots_pre_names[$pre_frame_pointer][$j] ne "" ) { 
			...
			unless ( $going ) {
				print "Problem matching spot $lines_spots_ann_names[$t][$k] in frame $frames_spots_ann[$t], proceeding without annotating this spot!\n";			
			}
		}
		#print "C: \$t = $t\n";
		#return;
	...
	if ( $out_xml ne "" ) {
		...
		if ( open( FILE, ">spotRadiusAnnotations_" . join( '', datetoday() ) . "T" . join( '', timenow() ) . ".txt"  ) ) {
			flock( FILE, LOCK_EX );
